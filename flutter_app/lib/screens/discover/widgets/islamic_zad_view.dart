...
/// Comprehensive, responsive Islamic Spiritual Knowledge Hub (Zad Al-Muslim).
/// Fully authenticated, non-controversial content with adaptive desktop/mobile layout.
class IslamicZadView extends StatefulWidget {
  final bool isDark;

  const IslamicZadView({super.key, required this.isDark});

  @override
  State<IslamicZadView> createState() => _IslamicZadViewState();
}

class _IslamicZadViewState extends State<IslamicZadView> {
  int _selectedSubTab = 0; // 0: Names of Allah, 1: Ruqyah & Duas, 2: Great Rewards, 3: Seerah Pearls
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.auto_stories_rounded, color: AppColors.goldBright, size: 20),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'نصوص موثقة من القرآن الكريم وصحيحي البخاري ومسلم وحصن المسلم',
        ...
        // Sub-Tabs Bar: أسماء الله | الرقية والأدعية | أجور مضاعفة | قبسات السيرة
        SingleChildScrollView(
          ...
          child: Container(
            ...
            child: Row(
              children: [
                _buildSubTab(0, Icons.favorite_border_rounded, 'أسماء الله الحسنى (${ZadDataConstants.allahNames.length})'),
                const SizedBox(width: 4),
                _buildSubTab(1, Icons.healing_rounded, 'الرقية والأدعية (${ZadDataConstants.ruqyahAndDuas.length})'),
                const SizedBox(width: 4),
                _buildSubTab(2, Icons.workspace_premium_rounded, 'أجور مضاعفة (${ZadDataConstants.greatRewardDeeds.length})'),
                const SizedBox(width: 4),
                _buildSubTab(3, Icons.menu_book_rounded, 'قبسات السيرة (${ZadDataConstants.propheticPearls.length})'),
              ],
            ),
          ),
        ),
        ...
        // Dua Category Chips (only for SubTab 1: Ruqyah & Duas)
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            final cards = _getFilteredCards(context, isDark, primaryColor);
            ...
            // Multi-column staggered layout for desktop / tablet
            final cols = List.generate(crossAxisCount, (_) => <Widget>[]);
            for (int i = 0; i < cards.length; i++) {
              cols[i % crossAxisCount].add(cards[i]);
              cols[i % crossAxisCount].add(const SizedBox(height: 14));
            }
  ...
  List<ZadItemCard> _getFilteredCards(BuildContext context, bool isDark, Color primaryColor) {
    final query = _searchQuery;

    if (_selectedSubTab == 0) {
      return ZadDataConstants.allahNames
          .where((item) =>
              query.isEmpty ||
