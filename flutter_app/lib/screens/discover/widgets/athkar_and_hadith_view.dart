...
/// Islamic Athkar & Hadith Encyclopedia Screen.
/// Coordinates Daily Athkar (with interactive countdowns) and the
/// Comprehensive Hadith Encyclopedia (Qudsi, Muttafaq, Nawawi, Riyad).
class AthkarAndHadithView extends StatefulWidget {
  final bool isDark;

  const AthkarAndHadithView({super.key, this.isDark = false});

  @override
  State<AthkarAndHadithView> createState() => _AthkarAndHadithViewState();
}

class _AthkarAndHadithViewState extends State<AthkarAndHadithView> {
  int _mainMode = 0; // 0: Daily Athkar, 1: Hadiths
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        // Mode Selector: الأذكار اليومية vs موسوعة الأحاديث النبوية
        Container(
          ...
          child: Row(
            children: [
              _buildMainTab(0, Icons.favorite_rounded, 'أذكار المسلم اليومية'),
              const SizedBox(width: 4),
              _buildMainTab(1, Icons.menu_book_rounded, 'موسوعة الحديث النبوي والصحيحين'),
            ],
          ),
        ),
        ...
        if (_mainMode == 0) ...[
          ...
          // ==================== موسوعة الأحاديث النبوية الشاملة ====================
          HadithEncyclopediaView(isDark: isDark),
        ],
      ],
    );
  }

  Widget _buildMainTab(int index, IconData icon, String title) {
        ...
        child: Container(
          ...
          child: FittedBox(
            ...
            child: Row(
              ...
              children: [
                Icon(icon, size: 18, color: isSelected ? Colors.white : (isDark ? Colors.white70 : Colors.black87)),
